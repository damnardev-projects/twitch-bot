...
class RaidConfigurationTwitchEnabledProcessorTests {

	@InjectMocks
	private RaidConfigurationTwitchEnabledProcessor raidConfigurationTwitchEnabledProcessor;
	...
	void process_shouldThrowException_whenRaidConfigurationNotFound() {
		// Given
		var form = ActionForm.UPDATE_RAID_TWITCH_SHOUTOUT.builder().resourceId(1L).build();

		given(this.findRaidConfigurationRepository.findByChannelId(1L)).willReturn(Optional.empty());

		// When
		var result = assertThatThrownBy(() -> this.raidConfigurationTwitchEnabledProcessor.process(form));
	...
	void process_shouldUpdateRaidConfiguration_whenRaidConfigurationFound(boolean value) {
		// Given
		var form = ActionForm.UPDATE_RAID_TWITCH_SHOUTOUT.builder().resourceId(1L).value(value).build();
		...
		// When
		this.raidConfigurationTwitchEnabledProcessor.process(form);

		// Then
		var expected = RaidConfiguration.builder().twitchShoutoutEnabled(value).build();
		then(this.findRaidConfigurationRepository).should().findByChannelId(1L);
