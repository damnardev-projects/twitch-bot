...
class RaidConfigurationWizebotEnabledProcessorTests {

	@InjectMocks
	private RaidConfigurationWizebotEnabledProcessor raidConfigurationWizebotEnabledProcessor;
	...
	void process_shouldThrowException_whenRaidConfigurationNotFound() {
		// Given
		var form = ActionForm.UPDATE_RAID_WIZEBOT_SHOUTOUT.builder().resourceId(1L).build();

		given(this.findRaidConfigurationRepository.findByChannelId(1L)).willReturn(Optional.empty());

		// When
		var result = assertThatThrownBy(() -> this.raidConfigurationWizebotEnabledProcessor.process(form));
	...
	void process_shouldUpdateRaidConfiguration_whenRaidConfigurationFound(boolean value) {
		// Given
		var form = ActionForm.UPDATE_RAID_WIZEBOT_SHOUTOUT.builder().resourceId(1L).value(value).build();
		...
		// When
		this.raidConfigurationWizebotEnabledProcessor.process(form);

		// Then
		var expected = RaidConfiguration.builder().wizebotShoutoutEnabled(value).build();
		then(this.findRaidConfigurationRepository).should().findByChannelId(1L);
